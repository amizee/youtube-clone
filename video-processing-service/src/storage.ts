...
const rawVideoBucketName = 'yt-clone-raw-videos';
const processedVideoBucketName = 'yt-clone-processed-videos';

const localRawVideoPath = './raw-videos';
const localProcessedVideoPath = './processed-videos';

/**
 * Creates the local directories for raw and processed videos, making sure the container can use them at the start
*/
  ...
  return new Promise<void>((resolve, reject) => {
    if (fs.existsSync(filePath)) {
      fs.unlink(filePath, (err) => {
        if (err) {
          console.log(`Failed to delete file at path ${filePath}`)
          reject(err);
        } else {
